...
First off, thank you for considering contributing to this project. It's people like you that make Appify Hub such a great tool.
...
Appify Hub is an open source project for a reason, and we love to receive contributions from our community — you!

There are many ways to contribute, from writing tutorials or blog posts, improving the documentation, submitting bug reports / bug fixes / feature requests, or even writing new code which can be incorporated into Appify Hub itself.
...
Not sure where to begin contributing to Appify Hub?

You can start by looking through the [list of open issues](https://github.com/appifyhub/monolith/issues).
...
In the case of Appify Hub, the project has traversed several phases:

  1. Side quest in an enterprise environment: Python + Django, closed-source
  1. Detached set of services in Digital Ocean: multi-framework, closed-source
      - User management: Go + Gin Gonic + Mongo DB
      - Authentication management: Kotlin + Ktor + Postgres + Exposed + Koin
      - Email management: Python + Flask + Apache WSGI + Mailgun
      - SMS management: Java + Spring Boot + Twilio
      - Push messaging: Firebase Functions + Firebase Firestore + Firebase Messaging + NodeJS + ExpressJS + PugJS
      - Web chat: Rust + Rocket
  1. Four services in a Docker-powered VPS: Kotlin + Spring Boot, closed-source
  1. Single service in a Kubernetes environment: Kotlin + Spring Boot, closed-source
  1. **The current, open-source version (Apr 2024)**: Kotin + Spring Boot

Different features and modules were created using various languages and approaches to problem-solving, as well as using different programming paradigms. Consequently, some of the current Kotlin code resembles JavaScript, other code looks like Python, and some tooling &and scripts are similar to how things are written in Rust.

A unified tech stack for all project layers is optimal, but constant and extensive refactoring was making this consolidation difficult.
But hey, here's the good news – the project now relies on **only one** technology: [Kotlin](https://kotlinlang.org).

In addition, it's important to note that [Chat GPT](https://openai.com/blog/chatgpt) contributed to parts of this code.
[GitHub's Copilot](https://github.com/features/copilot) also played a significant role with its code generation.
Due to the usage of these tools, coding styles and complexity may slightly vary across features.
...
The general how-to:
  ...
  1. Check the [issues](https://github.com/appifyhub/monolith/issues) to see if your change was already requested
  1. Create your own fork of the repository
  1. Make the changes inside of your fork
...
If you find yourself wishing for a feature that doesn't exist yet, you are probably not alone. There are others out there with similar needs. Many of the features that we have today have been added because the users saw the need for them, whether internally or externaly (post open-sourcing).
...
The rules are straightforward and more or less "industry standard":
  ...
  - We use imperative language for commit messages, e.g. "**Add** feature X" instead of "_Adding_ feature X"
