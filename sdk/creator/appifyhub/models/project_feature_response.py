...
class ProjectFeatureResponse(BaseModel):
    ...
    is_required: StrictBool = Field(description="Whether the feature is required for the project to run or not")
    __properties: ClassVar[List[str]] = ["name", "is_required"]

    @field_validator('name')
    def name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['BASIC', 'USERS', 'EMAILS', 'SMS', 'PUSH']):
            raise ValueError("must be one of enum values ('BASIC', 'USERS', 'EMAILS', 'SMS', 'PUSH')")
        return value
